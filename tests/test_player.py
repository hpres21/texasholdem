...
def test_npc_random(unused_parameter):
    """
    The random npc will choose FOLD, CALL, CHECK or BET a valid value no matter what are the hand and board.
    """
    ...
    if isinstance(p1.current_decision, int):
        assert p1.current_decision in list(range(2 * current_bet - p1.bet_this_round, p1.stack - p1.bet_this_round)) + [
...
def test_npc_strategy1_analysis(unused_parameter):
    """
    The analysis method will return a probability between 0 and 1 and its standard deviation.
    """
    # Initialize a random npc
    deck = Deck()
...
def test_npc_strategy1_decision(unused_parameter):
    """
    The npc with strategy 1 will only choose FOLD, CHECK or CALL (ALL IN if stack not enough to call).
    """
    # Initialize a random npc
    deck = Deck()
    stack = random.randint(0, 100)
    ...
    p1.decision(current_bet=current_bet, pot=pot, board=[deck.draw() for _ in range(5)])
    if isinstance(p1.current_decision, int):
        assert p1.current_decision in list(range(2 * current_bet - p1.bet_this_round, p1.stack - p1.bet_this_round)) + [
            0, current_bet - p1.bet_this_round]
    else:
...
def test_npc_strategy2_decision(unused_parameter):
    """
    The npc with strategy 2 will only choose FOLD, CHECK or CALL (ALL IN if stack not enough to call).
    """
    # Initialize a random npc
    deck = Deck()
    stack = random.randint(0, 100)
